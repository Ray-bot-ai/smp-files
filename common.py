...
3. 难以辨认的地方用 **〔此处转录不清〕** 整段标注一次即可，**不要一个字一个□地铺开**。
   一片模糊、一枚印章、一处污损，都只标一次，然后**继续往下转录**。
   切勿臆造，也不要用意思相近的字替代。
4. **一处看不清绝不能中断整页转录。** 页面各处是彼此独立的：
   开头的表头模糊、某枚印章不可辨、右上角被遮挡，都**不影响**其余部分——
   请把清晰的内容照常逐字转录完。实见模型在开头受阻后，把整页后续全标成不可辨，
   而原件其余部分其实非常清楚，这是必须避免的。
5. 繁体字、异体字、俗字保持原样，不要转成简体，也不要"规范化"。
6. 表格尽量用 Markdown 表格还原；印章、批注、页边小字可用「（印：…）」「（批：…）」标注。
   表单里用来填空的**点线、下划线**（如 Date . . . . . 19）不必照抄，
   写成「Date ____ 19」即可——照抄会让整页被点号淹没，正文反而出不来。
7. 不要输出"以下是识别结果"之类的话，也不要加代码围栏(```)。"""
