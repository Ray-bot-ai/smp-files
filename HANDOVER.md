...
6. **翻译/转录要防退化循环** —— 模型可能吐出成千上万次重复字符，撞 max_tokens 截断，
   导致**页码标记丢失、后续整页内容消失**，而且**不报任何错**，只表现为「某页特别长、某页没有」。
   `demo_run.degenerate()` 两道判据：同字符连续重复 ≥30 次；产出超源文 3.5 倍。
   命中就拆半重试，仍退化则留空，**绝不写入垃圾**。
7. **中文没有空格，CSS 必须 `overflow-wrap:anywhere`** —— 中共传单那页一行 782 字，
   `white-space:pre-wrap` 不会折行，直接撑破三栏布局。grid 子项还要加 `min-width:0`。
8. **OSS 签名 URL 绑死 HTTP 方法** —— `sign_url("GET",...)` 生成的 URL 用 `curl -I`(HEAD) 测会 403，
   那是测试写错了不是 OSS 有问题。要测就用 GET。
9. **百炼内容审查返回 HTTP 200，错误藏在 body 的 `error` 字段里** —— 脚本必须按这个判失败，
   不能只看状态码。
10. **改了公共配置会静默打断正在跑的后台任务** —— 本项目把 `common.py` 改成必须读环境变量后，
   正在跑的轮询循环读不到 key，空跑一小时才发现。现在 `common.py` 会读同目录 `.env`（已 gitignore）。
11. **页数别按 3,941 算** —— 其中 60 件 `smpa-N` 是缩微整卷，与散件内容重复。唯一件是 **3,878**。
